...
def patch_settings(monkeypatch):
    ...
    def set_value(db, key, value, description=""):
        db.values[key] = value
...
def test_daily_schedule_calculates_next_run(monkeypatch):
    ...
    db = FakeDB({"admin_timezone": "Asia/Kolkata"})
...
def test_one_time_schedule_requires_date(monkeypatch):
    ...
    db = FakeDB({"admin_timezone": "Asia/Kolkata"})
    try:
        schedule_service.configure_schedule(db, "email", frequency="once", date=None, time="20:00")
...
def test_claim_advances_daily_schedule(monkeypatch):
    ...
    claimed = schedule_service.claim_due(db, "email", datetime.datetime(2026, 9, 3, 15, 0, tzinfo=datetime.timezone.utc))
    assert claimed is not None
    assert claimed["last_status"] == "in_progress"
    assert claimed["next_run_at"].startswith("2026-09-04T14:30:00")
