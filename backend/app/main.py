...
app = FastAPI(
    ...
    description="LLM-powered daily news digest - open source stack (FastAPI + Groq + free RSS).",
    version="1.0.0",
)
...
# ---------- Global error handling: never leak a raw 500 stack trace to the client ----------
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    # Return a cleaner, user-friendly shape than FastAPI's default verbose error
...
def on_startup():
    ...
    try:
        ...
        logger.error(f"Seeding failed (app will still start): {e}")
    finally:
        db.close()
