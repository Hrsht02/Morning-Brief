...
def _localize_for_email(story,language):
 ...
 if language=="hi" and story.headline_hi and story.summary_hi:item.headline=story.headline_hi;item.hook=story.hook_hi or story.hook;item.summary=story.summary_hi
 return item

def _get_approved_stories_for_edition(db:Session,edition_date:str):
 if not edition_date:return []
...
def _get_latest_approved_edition_date(db:Session,today=None):
 today=today or datetime.datetime.now(datetime.timezone.utc).date();row=db.query(models.Story.edition_date).filter(models.Story.edition_date<=today.isoformat(),models.Story.is_published.is_(True),models.Story.publication_status=="approved",models.Story.is_test_content.is_(False)).order_by(models.Story.edition_date.desc()).first();return row[0] if row else None
...
def send_daily_emails(db:Session,force=False):
 ...
 db.commit();return {"status":"completed","sent":sent,"failed":failed,"skipped":skipped,"edition_date":edition,"email_top_n":top_n,"skip_reasons":skip_reasons}

def send_test_email(db:Session,test_recipient:str,language="en"):
 edition=datetime.datetime.now(datetime.timezone.utc).astimezone(admin_timezone(db)).date().isoformat();stories=_get_approved_stories_for_edition(db,edition);top_n=max(1,int(get_setting(db,"email_top_n","25")))
 if not stories:return {"status":"skipped","detail":f"No approved production stories are available for today ({edition})"}
 try:html=render_digest_email([_localize_for_email(s,language) for s in stories[:top_n]],edition,settings.FRONTEND_URL,max_stories=min(top_n,len(stories)));send_email(to_email=test_recipient,subject=f"[TEST] Your Morning Brief — {edition}",html_content=html);return {"status":"completed","detail":f"Test email sent to {test_recipient} using today's approved edition {edition}","stories_sent":min(top_n,len(stories))}
