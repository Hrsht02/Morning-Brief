...
def send_daily_emails(db: Session, force=False):
    ...
    for user in users:
        ...
        if _already_delivered_current_content(db, user.id, edition_date, stories): skipped += 1; skip_reasons["already_delivered"] += 1; continue
        ...
        try:
            html = render_digest_email(localized, edition_date, settings.FRONTEND_URL, max_stories=len(localized)); send_email(to_email=user.email, subject=f"Your Morning Brief — {edition_date}", html_content=html)
            user.last_sent_date = edition_date; db.add(models.EmailLog(user_id=user.id, edition_date=edition_date, status="sent")); sent += 1
        except EmailSendError as exc:
            db.add(models.EmailLog(user_id=user.id, edition_date=edition_date, status="failed", error=str(exc)[:500])); failed += 1
        except Exception as exc:
            logger.exception("Unexpected email error for user %s", user.id); db.add(models.EmailLog(user_id=user.id, edition_date=edition_date, status="failed", error=str(exc)[:500])); failed += 1
    db.commit(); return {"status": "completed", "sent": sent, "failed": failed, "skipped": skipped, "skip_reasons": skip_reasons}
...
def send_test_email(db: Session, test_recipient: str, language: str = "en"):
    ...
    try:
        html = render_digest_email(localized, edition_date, settings.FRONTEND_URL, max_stories=len(localized)); send_email(to_email=test_recipient, subject=f"[TEST] Your Morning Brief — {edition_date}", html_content=html)
