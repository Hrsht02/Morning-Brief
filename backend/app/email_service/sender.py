"""Timezone-aware, country/category-personalized email delivery."""
import datetime
import logging
import zoneinfo
from sqlalchemy.orm import Session, joinedload
from .. import models
from ..seed import get_setting
from ..services.personalization import select_personalized_stories
from .brevo_client import send_email, render_digest_email, EmailSendError
from ..config import settings

logger = logging.getLogger("morning_brief.email")


def _is_users_send_time_now(user, window_minutes=60):
    try: tz = zoneinfo.ZoneInfo(user.timezone)
    except Exception: tz = zoneinfo.ZoneInfo("UTC")
    now = datetime.datetime.now(tz)
    target = now.replace(hour=user.send_hour, minute=user.send_minute, second=0, microsecond=0)
    return 0 <= (now - target).total_seconds() / 60 < window_minutes


def _localize_for_email(story, language):
    if language == "hi" and story.headline_hi and story.summary_hi:
        story.headline, story.hook, story.summary = story.headline_hi, story.hook_hi or story.hook, story.summary_hi
    return story


def _get_todays_approved_stories(db, today):
    return db.query(models.Story).options(joinedload(models.Story.citations)).filter(
        models.Story.edition_date == today, models.Story.is_published.is_(True),
        models.Story.publication_status == "approved", models.Story.is_test_content.is_(False)
    ).all()


def send_daily_emails(db: Session, force=False):
    # The backend determines each user's local date, so a UTC cron tick never
    # causes a reader to receive yesterday's edition accidentally.
    users = db.query(models.User).filter(models.User.is_active.is_(True), models.User.onboarded.is_(True)).all()
    sent = failed = skipped = 0
    for user in users:
        try:
            tz = zoneinfo.ZoneInfo(user.timezone)
        except Exception: tz = zoneinfo.ZoneInfo("UTC")
        local_today = datetime.datetime.now(tz).date().isoformat()
        stories = _get_todays_approved_stories(db, local_today)
        if not stories:
            skipped += 1; continue
        if user.last_sent_date == local_today and not force:
            skipped += 1; continue
        if not force and not _is_users_send_time_now(user, window_minutes=60):
            skipped += 1; continue

        selected, _ = select_personalized_stories(
            stories, user.country_code, {c.category_slug for c in user.categories},
            int(get_setting(db, "stories_per_edition", "8")),
            int(get_setting(db, "outside_bubble_min_stories", "1")),
        )
        localized = [_localize_for_email(s, user.content_language) for s in selected]
        try:
            html = render_digest_email(localized, local_today, settings.FRONTEND_URL, max_stories=len(localized))
            send_email(to_email=user.email, subject=f"Your Morning Brief — {local_today}", html_content=html)
            user.last_sent_date = local_today
            db.add(models.EmailLog(user_id=user.id, edition_date=local_today, status="sent")); sent += 1
        except EmailSendError as exc:
            db.add(models.EmailLog(user_id=user.id, edition_date=local_today, status="failed", error=str(exc)[:500])); failed += 1
        except Exception as exc:
            logger.exception("Unexpected email error for user %s", user.id)
            db.add(models.EmailLog(user_id=user.id, edition_date=local_today, status="failed", error=str(exc)[:500])); failed += 1
    db.commit()
    return {"status":"ok", "sent":sent, "failed":failed, "skipped":skipped}


def send_test_email(db: Session, test_recipient: str, language: str = "en"):
    today = datetime.date.today().isoformat()
    stories = _get_todays_approved_stories(db, today)
    if not stories: return {"status":"skipped", "detail":"No published stories for today yet"}
    localized = [_localize_for_email(s, language) for s in stories[:int(get_setting(db, "stories_per_edition", "8"))]]
    try:
        html = render_digest_email(localized, today, settings.FRONTEND_URL, max_stories=len(localized))
        send_email(to_email=test_recipient, subject=f"[TEST] Your Morning Brief — {today}", html_content=html)
        return {"status":"ok", "detail":f"Test email sent to {test_recipient}"}
    except Exception as exc: return {"status":"error", "detail":str(exc)[:500]}
