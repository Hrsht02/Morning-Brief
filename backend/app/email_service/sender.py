...
def _is_users_send_time_now(user, window_minutes=60):
    ...
    now = datetime.datetime.now(tz); target = now.replace(hour=6, minute=0, second=0, microsecond=0); return 0 <= (now - target).total_seconds() / 60 < window_minutes


def _localize_for_email(story, language):
    if language == "hi" and story.headline_hi and story.summary_hi: story.headline, story.hook, story.summary = story.headline_hi, story.hook_hi or story.hook, story.summary_hi
    return story
...
def _get_latest_approved_edition_date(db: Session, local_today: datetime.date) -> str | None:
    row = db.query(models.Story.edition_date).filter(models.Story.edition_date == local_today.isoformat(), models.Story.is_published.is_(True), models.Story.publication_status == "approved", models.Story.is_test_content.is_(False)).order_by(models.Story.edition_date.desc()).first(); return row[0] if row else None
...
def send_daily_emails(db: Session, force=False):
    """Send today's approved production edition only to explicit email opt-ins."""
    ...
    sent = failed = skipped = 0; skip_reasons = {"no_today_edition": 0, "already_delivered": 0, "no_stories": 0, "no_personalized_stories": 0, "no_email_consent": 0}
    admin_today = datetime.datetime.now(datetime.timezone.utc).astimezone(admin_timezone(db)).date().isoformat(); stories = _get_approved_stories_for_edition(db, admin_today)
    for user in users:
        try:
            consent = db.query(compliance_models.UserConsent).filter_by(user_id=user.id).first()
            if not consent or not consent.email_news_opt_in:
                skipped += 1; skip_reasons["no_email_consent"] += 1; continue
            if not stories:
...
def send_test_email(db: Session, test_recipient: str, language: str = "en"):
    ...
    try:
        html = render_digest_email(localized, today, settings.FRONTEND_URL, max_stories=len(localized)); send_email(to_email=test_recipient, subject=f"[TEST] Your Morning Brief — {today}", html_content=html); return {"status": "completed", "detail": f"Test email sent to {test_recipient} using today's approved edition {today}"}
