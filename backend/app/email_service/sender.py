...
def _get_latest_approved_edition_date(db:Session,today=None):
 ...
 row=db.query(models.Story.edition_date).filter(models.Story.edition_date<=today.isoformat(),models.Story.is_published.is_(True),models.Story.publication_status=="approved",models.Story.is_test_content.is_(False)).order_by(models.Story.edition_date.desc()).first()
