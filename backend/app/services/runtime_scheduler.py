...
def _heartbeat(db, key, value):
    try:
        ...
        db.rollback()
        logger.exception("Unable to persist scheduler state %s", key)
...
def _run_email_worker(schedule):
    ...
    try:
        ...
        _heartbeat(db, "scheduler_last_email_start", datetime.datetime.now(datetime.timezone.utc).isoformat())
        ...
        if result.get("status") == "completed":
            ...
            finish_schedule(db, "email", "completed", result)
        else:
            fail_job(db, "email", result.get("detail", "Scheduled email did not complete"), result)
            finish_schedule(db, "email", "failed", result)
    except Exception as exc:
        logger.exception("Scheduled email failed")
        fail_job(db, "email", str(exc))
        finish_schedule(db, "email", "failed", {"error": str(exc)})
    finally:
        db.close()
        _locks["email"].release()
...
def _run_ingestion_worker(schedule):
    try:
        ...
        try:
            ...
            _heartbeat(db, "scheduler_last_ingestion_start", datetime.datetime.now(datetime.timezone.utc).isoformat())
        finally:
            db.close()
        ...
        try:
            ...
            if result.get("status") == "completed":
                finish_schedule(db, "ingestion", "completed", result.get("result"))
            else:
                finish_schedule(db, "ingestion", "failed", result.get("result") or {"error": result.get("error")})
        finally:
            db.close()
    except Exception as exc:
...
def run_due_production_job(job_type: str) -> dict:
    ...
    if job_type not in {"email", "ingestion"}:
        return {"status": "error", "detail": "Unsupported production job"}
    db = SessionLocal()
    acquired = False
    try:
        if get_setting(db, "scheduling_mode", "auto").lower() != "auto":
            return {"status": "skipped", "detail": "Automatic scheduling is disabled"}
        if get_job(db, job_type).get("status") == "in_progress":
            return {"status": "in_progress", "detail": f"{job_type} is already running"}
        acquired = _locks[job_type].acquire(blocking=False)
        if not acquired:
            return {"status": "in_progress", "detail": f"{job_type} is already being claimed"}
        schedule = claim_due(db, job_type)
        if not schedule:
            _locks[job_type].release()
            acquired = False
            return {"status": "waiting", "detail": "No due schedule"}
...
def _tick():
    ...
    db = SessionLocal()
    try:
        _heartbeat(db, "scheduler_last_tick_at", now)
    finally:
        db.close()
    ...
    for job_type in ("ingestion", "email"):
        try:
            ...
            if result.get("status") == "in_progress":
                db = SessionLocal()
                try:
                    _heartbeat(db, f"scheduler_last_trigger_{job_type}", now)
                finally:
                    db.close()
        except Exception:
            logger.exception("%s scheduler tick failed", job_type)
