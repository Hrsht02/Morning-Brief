...
SENSITIVE_RULES = {
    ...
    "election": re.compile(r"\b(election|elections|polling|poll|candidate|exit poll|vote|voting|ballot|constituency|lok sabha|assembly election|election commission)\b", re.I),
    "communal_religious_caste": re.compile(r"\b(communal|religious|religion|hindu|muslim|christian|sikh|caste|dalit|minority|temple|mosque|church|gurdwara)\b", re.I),
...
def mandatory_human_review(text: str, source_count: int, verifier_report: dict | None,
    ...
    A single-source story is recorded for editorial awareness but is not by
    itself a mandatory-review trigger. Similarity is never a legal safe harbor.
    """
