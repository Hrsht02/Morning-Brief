...
so the requested time is a target and may be delivered within a few minutes.
"""
...
def schedule_test_email(db: Session, hour: int, minute: int) -> dict:
    ...
    if not recipient:
        return {"status": "error", "detail": "developer_test_email is not configured", "safe": True}
...
def get_test_schedule(db: Session) -> dict:
    return {
        ...
        "scheduled_at": get_setting(db, SCHEDULED_AT_KEY, "") or None,
