"""Fetch and normalize active RSS sources with per-source isolation and diagnostics."""
...
def fetch_all_active_sources(db: Session, blocked_domains=None, published_after: datetime.datetime | None = None):
    ...
    articles=[]; active_count=0; successful_count=0; errors=[]
    for source in db.query(models.Source).filter(models.Source.is_active.is_(True)).all():
        ...
        try:
            ...
            source.last_fetched_at=datetime.datetime.utcnow(); source.last_fetch_error=None; successful_count += 1
            logger.info("Fetched %s entries from %s",count,source.name)
        except Exception as exc:
            message=str(exc)[:500]; source.last_fetch_error=message; errors.append({"source":source.name,"error":message}); logger.warning("Failed to fetch source '%s': %s",source.name,exc)
    db.commit()
    if active_count == 0:
        raise RuntimeError("No active RSS sources are configured. Activate at least one source in Admin → Sources.")
    if successful_count == 0:
        detail="All active RSS sources failed to fetch. " + "; ".join(f"{item['source']}: {item['error']}" for item in errors[:10])
        raise RuntimeError(detail[:2000])
    return articles, {"active_sources":active_count,"successful_sources":successful_count,"failed_sources":len(errors),"errors":errors[:20]}
