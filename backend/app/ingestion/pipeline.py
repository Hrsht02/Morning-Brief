"""Production ingestion: fetch -> cluster -> generate -> originality -> verify -> publish."""
import datetime
import json
import logging
import time
...
def _persist_story_with_retry(story_data,articles):
    """Persist one story in a short, disposable transaction.

    LLM/network work never holds the database transaction open. A transient
    PostgreSQL SSL disconnect therefore gets a fresh Session/connection and
    does not poison the rest of the ingestion run.
    """
    last_error=None
    for attempt in range(1,DB_WRITE_RETRIES+1):
        ...
        try:
            ...
            write_db.add(story);write_db.flush()
            seen=set()
            for article in articles:
                if article.link in seen:continue
                seen.add(article.link)
                write_db.add(models.Citation(story_id=story.id,source_name=article.source_name,title=article.title,url=article.link))
            write_db.commit()
            return story.id
        except OperationalError as exc:
            last_error=exc
            write_db.rollback()
            logger.warning("Transient database error saving story (attempt %s/%s): %s",attempt,DB_WRITE_RETRIES,exc)
...
def _cancel_requested():
    """Read the cancellation flag through a fresh short-lived DB session."""
    db=SessionLocal()
...
def _heartbeat(stage,cluster_index=0,total_clusters=0):
    """Persist live progress without holding the main ingestion transaction."""
    db=SessionLocal()
    try:
        set_setting(db,"ingestion_heartbeat_at",datetime.datetime.utcnow().isoformat())
        set_setting(db,"ingestion_current_stage",stage)
        set_setting(db,"ingestion_current_cluster",str(cluster_index))
        set_setting(db,"ingestion_total_clusters",str(total_clusters))
        db.commit()
    except Exception:
        db.rollback()
    finally:db.close()
...
def _check_cancel(stage,cluster_index=0,total_clusters=0):
    ...
    if _cancel_requested():
        raise IngestionCancelled(f"Ingestion stopped by administrator during {stage}")
...
def run_ingestion(db:Session,test_mode=False,max_clusters=None,freshness_after=None):
    ...
    configured_auto_threshold=float(get_setting(db,"auto_approval_similarity_threshold","0.20"))
    auto_approval_threshold=min(configured_auto_threshold,ORIGINALITY_TARGET)
    ...
    stage="rss_fetch"
    _check_cancel(stage,0,0)
    try:
        articles,fetch_diagnostics=fetch_all_active_sources(db,blocked_domains=blocked,published_after=freshness_after)
        db.commit()
    except IngestionCancelled:raise
    except Exception as exc:
        db.rollback();logger.exception("Fetch stage failed: %s",exc);return {"status":"error","stage":stage,"detail":str(exc),"stories_created":0}
    ...
    cluster_limit=(max_clusters or 3) if test_mode else (max_clusters or int(get_setting(db,"max_clusters_per_run","100")))
    clusters=clusters[:cluster_limit]
    total_clusters=len(clusters)
    _check_cancel("clustering_complete",0,total_clusters)
    if not test_mode:
        try:
            db.query(models.Story).filter(models.Story.edition_date==today,models.Story.is_pinned.is_(False),models.Story.publication_status!="approved",models.Story.is_test_content.is_(False)).delete(synchronize_session=False);db.commit()
        except Exception:db.rollback();raise
    created=0;pause=max(0,float(get_setting(db,"llm_pause_seconds","1")));cluster_errors=[]
    for i,cluster in enumerate(clusters):
        cluster_no=i+1
        _check_cancel(f"starting_cluster_{cluster_no}",cluster_no,total_clusters)
        snippets=[{"title":a.title,"summary":a.summary,"source":a.source_name} for a in cluster.articles]
        stage=f"generation_cluster_{cluster_no}"
        ...
        _check_cancel(f"generation_complete_cluster_{cluster_no}",cluster_no,total_clusters)
        originals=[f"{a.title} {a.summary}" for a in cluster.articles];rewrite_count=0
        if not skip_verification:
            try:
                for rewrite_no in range(MAX_ORIGINALITY_REWRITES):
                    _check_cancel(f"originality_cluster_{cluster_no}_pass_{rewrite_no+1}",cluster_no,total_clusters)
                    current_similarity=compute_max_similarity(draft["summary"],originals)
                    if current_similarity<ORIGINALITY_TARGET:break
                ...
                logger.warning("Originality rewrite check failed: %s",exc);cluster_errors.append({"cluster":cluster_no,"stage":"originality","error":str(exc)[:500]})
        ...
        try:
            ...
            if skip_verification:
                flags=[];max_similarity=0.0;long_phrase=0.0;report=None;verification_blocked=False
            else:
                context={"cluster_articles":cluster.articles,"draft":draft,"original_snippets":originals,"thresholds":thresholds,"blocked_domains":blocked}
                result=run_verification_pipeline(layers,context)
                flags=result["all_flags"];max_similarity=context.get("max_similarity",0.0);long_phrase=context.get("max_long_phrase_overlap",0.0);report=context.get("verifier_report")
            ...
            logger.exception("Verification stage failed for cluster %s",cluster_no);return {"status":"error","stage":stage,"detail":str(exc),"stories_created":created,"fetch_diagnostics":fetch_diagnostics,"cluster_errors":cluster_errors}
        _check_cancel(f"verification_complete_cluster_{cluster_no}",cluster_no,total_clusters)
        countries={getattr(a,"country_code","GLOBAL") for a in cluster.articles};country=next(iter(countries)) if len(countries)==1 else "GLOBAL"
        story_data={"edition_date":today,"headline":draft["headline"],"hook":draft["hook"],"summary":draft["summary"],"headline_hi":draft.get("headline_hi"),"hook_hi":draft.get("hook_hi"),"summary_hi":draft.get("summary_hi"),"category_slug":draft["category_slug"],"country_code":country,"confidence_score":draft["confidence"],"needs_review":status=="pending","is_published":published,"is_test_content":test_mode,"publication_status":status,"pipeline_stage":"pending_human_review" if status=="pending" else "published","verification_flags":json.dumps(flags) if flags else None,"max_source_similarity":max_similarity,"max_long_phrase_overlap":long_phrase,"originality_rewrite_applied":rewrite_count>0,"generator_model":f"groq:{settings.GROQ_MODEL}","verifier_model":f"gemini:{settings.GEMINI_MODEL}" if report else None,"verifier_report":json.dumps(report) if report else None,"contradiction_flag":bool(report and report.get("contradiction_found")),"citation_complete":bool(cluster.articles)}
        try:_persist_story_with_retry(story_data,cluster.articles)
        except Exception as exc:
            logger.exception("Story persistence failed for cluster %s",cluster_no);return {"status":"error","stage":"database_persist","detail":str(exc)[:1000],"stories_created":created,"fetch_diagnostics":fetch_diagnostics,"cluster_errors":cluster_errors}
        created+=1
        _check_cancel(f"completed_cluster_{cluster_no}",cluster_no,total_clusters)
        if not test_mode and i<len(clusters)-1:time.sleep(pause)
    return {"status":"ok","stage":"completed","detail":f"Processed {len(clusters)} clusters; verified publication requires similarity below {ORIGINALITY_TARGET:.0%}; drafts at/above target are rewritten up to {MAX_ORIGINALITY_REWRITES} times","stories_created":created,"freshness_after":freshness_after.isoformat() if freshness_after else None,"fetch_diagnostics":fetch_diagnostics,"cluster_errors":cluster_errors}
...
def _stale_running(db):
    ...
    try:
        started=datetime.datetime.fromisoformat(raw.replace("Z","+00:00"));started=started.replace(tzinfo=datetime.timezone.utc) if started.tzinfo is None else started
...
def run_ingestion_background(mode="manual",freshness_after=None):
    ...
    try:
        ...
        set_setting(db,"ingestion_cancel_requested","false")
        set_setting(db,"ingestion_current_stage","starting")
        set_setting(db,"ingestion_current_cluster","0")
        set_setting(db,"ingestion_total_clusters","0")
        set_setting(db,"ingestion_heartbeat_at",datetime.datetime.utcnow().isoformat())
        start_job(db,"ingestion",mode=mode);set_setting(db,"ingestion_status","running","Current background ingestion status");set_setting(db,"ingestion_started_at",datetime.datetime.utcnow().isoformat());set_setting(db,"ingestion_completed_at","");db.commit()
        ...
        logger.info("Ingestion cancelled: %s",exc)
        db.rollback()
        result={"status":"cancelled","stage":"cancelled","detail":str(exc)}
        try:
            set_setting(db,"ingestion_status","cancelled");set_setting(db,"ingestion_last_result",json.dumps(result));set_setting(db,"ingestion_completed_at",datetime.datetime.utcnow().isoformat());set_setting(db,"ingestion_cancel_requested","false");db.commit();cancel_job(db,"ingestion",result)
