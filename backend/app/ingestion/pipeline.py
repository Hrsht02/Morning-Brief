...
def run_ingestion(db:Session,test_mode=False,max_clusters=None):
    ...
    for i,cluster in enumerate(clusters):
        ...
        draft=summarize_cluster(snippets,categories,max_sentences=max_sentences,bilingual=bilingual)
        originals=[f"{a.title} {a.summary}" for a in cluster.articles]; rewrite=False
        try:
        ...
        if skip_verification:
            flags=[]; status="approved"; published=True; max_similarity=0.0; long_phrase=0.0; report=None
        else:
            context={"cluster_articles":cluster.articles,"draft":draft,"original_snippets":originals,"thresholds":thresholds,"blocked_domains":blocked}; result=run_verification_pipeline(layers,context)
            ...
            status="pending" if result["must_hold"] or flags or require_human else "approved"; published=status=="approved"
