...
def run_ingestion(db:Session,test_mode=False,max_clusters=None,freshness_after=None):
    ...
    return {"status":"ok","stage":"completed","detail":f"Ranked {raw_article_count} fetched articles down to {len(articles)} candidates, then processed {len(clusters)} clusters; publication requires similarity below {ORIGINALITY_TARGET:.0%}","stories_created":created,"freshness_after":freshness_after.isoformat() if freshness_after else None,"fetch_diagnostics":fetch_diagnostics,"cluster_errors":cluster_errors}
