...
def run_ingestion(db:Session,test_mode=False,max_clusters=None,freshness_after=None):
    ...
    for i,cluster in enumerate(clusters):
        ...
        except Exception as exc:logger.exception("Generation failed for cluster %s: %s",i+1,exc);return {"status":"error","stage":stage,"detail":str(exc),"stories_created":created,"fetch_diagnostics":fetch_diagnostics,"cluster_errors":cluster_errors}
        originals=[f"{a.title} {a.summary}" for a in cluster.articles]
        rewrite_count=0
        rewrite_similarity=None
        if not skip_verification:
            # Do not merely change the approval cutoff. Actively rewrite any
            # draft at/above 20%, then measure the result. Up to three passes
            # are allowed so ordinary drafts have a real chance to become
            # sufficiently original before being routed to human review.
            try:
                for attempt in range(MAX_ORIGINALITY_REWRITES):
                    current_similarity=compute_max_similarity(draft["summary"],originals)
                    rewrite_similarity=current_similarity
                    ...
                    rewrite_count+=1
                    rewrite_similarity=compute_max_similarity(draft["summary"],originals)
                    if rewrite_similarity < ORIGINALITY_TARGET:
                        break
    ...
    db.commit();return {"status":"ok","stage":"completed","detail":f"Processed {len(clusters)} clusters; automatic approval target {ORIGINALITY_TARGET:.0%}; drafts at/above target are rewritten up to {MAX_ORIGINALITY_REWRITES} times","stories_created":created,"freshness_after":freshness_after.isoformat() if freshness_after else None,"fetch_diagnostics":fetch_diagnostics,"cluster_errors":cluster_errors}
