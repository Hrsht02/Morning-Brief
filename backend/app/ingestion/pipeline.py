...
def run_ingestion(db:Session,test_mode=False,max_clusters=None,freshness_after=None):
    ...
    for i,cluster in enumerate(clusters):
        ...
        stage=f"generation_cluster_{i+1}"
        ...
        if not skip_verification:
            try:
                for _ in range(MAX_ORIGINALITY_REWRITES):
        ...
        try:
            ...
            logger.exception("Verification stage failed for cluster %s",i+1);return {"status":"error","stage":stage,"detail":str(exc),"stories_created":created,"fetch_diagnostics":fetch_diagnostics,"cluster_errors":cluster_errors}
        ...
        try:_persist_story_with_retry(story_data,cluster.articles)
            ...
            logger.exception("Story persistence failed for cluster %s",i+1);return {"status":"error","stage":"database_persist","detail":str(exc)[:1000],"stories_created":created,"fetch_diagnostics":fetch_diagnostics,"cluster_errors":cluster_errors}
        created+=1
...
def run_ingestion_background(mode="manual",freshness_after=None):
    ...
    try:
        ...
        try:set_setting(db,"ingestion_status","error");set_setting(db,"ingestion_last_result",json.dumps(result));set_setting(db,"ingestion_completed_at",datetime.datetime.utcnow().isoformat());db.commit();fail_job(db,"ingestion",error,result)
