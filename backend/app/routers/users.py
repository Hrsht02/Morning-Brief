...
from ..services.personalization import SUPPORTED_COUNTRIES, normalize_country, resolve_country
...
def _user_out(user: models.User) -> schemas.UserOut:
    return schemas.UserOut(
        ...
        country_code=user.country_code, timezone=user.timezone,
        send_hour=user.send_hour, send_minute=user.send_minute,
...
def supported_countries():
    return {
        "countries": [{"code": code, "name": name} for code, name in SUPPORTED_COUNTRIES.items() if code != "GLOBAL"],
        "fallback": {"code": "GLOBAL", "name": "Global"},
        "default": "IN",
    }
...
def complete_onboarding(payload: schemas.OnboardingRequest, db: Session = Depends(get_db), user: models.User = Depends(get_current_user)):
    ...
    country = normalize_country(payload.country_code)
    if not country:
        raise HTTPException(status_code=400, detail="Unsupported country code. Choose a supported country or GLOBAL fallback.")
