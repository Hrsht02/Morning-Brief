...
def job_status(db:Session=Depends(get_db),_=Depends(get_current_developer_or_admin)):
 ...
 progress={"stage":get_setting(db,"ingestion_current_stage","") or None,"current_cluster":max(0,int(get_setting(db,"ingestion_current_cluster","0") or 0)),"total_clusters":max(0,int(get_setting(db,"ingestion_total_clusters","0") or 0)),"heartbeat_at":get_setting(db,"ingestion_heartbeat_at","") or None,"cancel_requested":get_setting(db,"ingestion_cancel_requested","false").lower()=="true"}
