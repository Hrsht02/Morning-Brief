...
def job_status(db:Session=Depends(get_db),_=Depends(get_current_developer_or_admin)):
    ...
    return {"ingestion":{"status":get_setting(db,"ingestion_status","idle"),"started_at":get_setting(db,"ingestion_started_at",""),"completed_at":get_setting(db,"ingestion_completed_at",""),"last_result":last},"schedule":{"mode":get_setting(db,"scheduling_mode","auto"),"admin_timezone":get_setting(db,"admin_timezone","Asia/Kolkata"),"email_window":[get_setting(db,"email_send_window_start","06:00"),get_setting(db,"email_send_window_end","07:00")]}}
