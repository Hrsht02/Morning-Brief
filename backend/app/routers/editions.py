...
def get_edition(date: Optional[str] = Query(default=None), category: Optional[str] = Query(default=None),
    ...
    stories = db.query(models.Story).options(joinedload(models.Story.citations)).filter(
        ...
        models.Story.is_published.is_(True), models.Story.publication_status == "approved",
    ...
    if category and category != "general":
        stories = [s for s in stories if s.category_slug == category][:limit]
    else:
        stories, resolution = select_personalized_stories(
            stories, user.country_code, {c.category_slug for c in user.categories}, limit,
...
def get_story_detail(story_id: int, db: Session = Depends(get_db), user: models.User = Depends(get_current_user)):
    story = db.query(models.Story).options(joinedload(models.Story.citations)).filter(
        models.Story.id == story_id, models.Story.is_published.is_(True),
...
def _build_edition_response(edition_date, stories, language, resolution):
    ...
    return schemas.EditionOut(edition_date=edition_date, story_count=len(trimmed),
        estimated_read_minutes=max(1, round(len(trimmed) * 0.5)), stories=trimmed,
        country_requested=resolution.requested or None, country_effective=resolution.effective,
        country_supported=resolution.supported, fallback_used=resolution.fallback_used)
