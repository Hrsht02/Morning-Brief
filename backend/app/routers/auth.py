...
def signup(payload:schemas.SignupRequest,db:Session=Depends(get_db)):
 ...
 user=models.User(email=payload.email.lower(),hashed_password=hash_password(payload.password),auth_provider="password",plan_id=_default_plan_id(db));db.add(user);db.flush();_ensure_consent_row(db,user.id,True);db.commit();db.refresh(user);return schemas.TokenResponse(access_token=create_access_token(user.id))
...
def login(payload:schemas.LoginRequest,db:Session=Depends(get_db)):
 ...
 _ensure_consent_row(db,user.id,False);db.commit();return schemas.TokenResponse(access_token=create_access_token(user.id))
