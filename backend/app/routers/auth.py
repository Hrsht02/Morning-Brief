...
def _ensure_consent_row(db: Session,user_id:int):
    ...
    if row is None:
        # New accounts start with Morning Brief email delivery enabled; the
        # user can immediately turn it off from Preferences.
        db.add(compliance_models.UserConsent(user_id=user_id,email_news_opt_in=True))
...
def signup(payload:schemas.SignupRequest,db:Session=Depends(get_db)):
    ...
    user=models.User(email=payload.email.lower(),hashed_password=hash_password(payload.password),auth_provider="password",plan_id=_default_plan_id(db));db.add(user);db.flush();_ensure_consent_row(db,user.id);db.commit();db.refresh(user);return schemas.TokenResponse(access_token=create_access_token(user.id))
...
def login(payload:schemas.LoginRequest,db:Session=Depends(get_db)):
    ...
    _ensure_consent_row(db,user.id);db.commit();return schemas.TokenResponse(access_token=create_access_token(user.id))
...
def google_login(payload:schemas.GoogleLoginRequest,db:Session=Depends(get_db)):
    ...
    if user is None:
        ...
        else:user=models.User(email=email,hashed_password=generate_random_password_hash(),auth_provider="google",google_sub=google_sub,plan_id=_default_plan_id(db));db.add(user);db.flush()
