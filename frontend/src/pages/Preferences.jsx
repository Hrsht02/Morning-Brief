...
export default function Preferences() {
  ...
  const handleSave = async () => { setSaving(true); setError(""); setMessage(""); try { const timezone = user?.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone || "UTC"; await api.put("/users/preferences", { timezone, country_code: country, send_hour: 6, send_minute: 0, category_slugs: selected, content_language: language }); await refreshUser(); setMessage("Preferences saved. Your Morning Brief arrives at 6:00 AM local time."); } catch (err) { setError(err.friendlyMessage || "Couldn't save preferences"); } finally { setSaving(false); } };
  if (loading) return <Loader text="Loading your preferences..." />;
  return <div className="onboarding-page">
    <div className="onboarding-step-title">Your preferences</div><div className="onboarding-step-sub">Your country affects local-news ranking. Your timezone determines the 6:00 AM delivery time.</div>
    ...
    <div className="onboarding-step-title" style={{ fontSize: 20 }}>Delivery</div><div className="onboarding-step-sub" style={{ marginBottom: 16 }}>Every subscriber receives the briefing at <strong>6:00 AM local time</strong>. This keeps delivery predictable and reliable.</div>
